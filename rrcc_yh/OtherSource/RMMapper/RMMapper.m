...
//            NSLog(@"TDUtils: key %@ is skipped", key);
        ...
        if ([propertyType isEqualToString:@"NSString"] \
            ...
            // Else, set value for key
                ...
                if (value == nil || [value isEqual:@"<null>"] || [value isEqual:[NSNull null]] || [value isEqual:@"null"])
                {
    ...
    for(id pKey in properties)
    {
        id pValue = [obj valueForKey:pKey];
        if (pValue) {
            
        }else{
//            [obj setValue:@"" forKey:pKey];
            if ([[properties objectForKey:pKey] isEqualToString:@"NSString"]) {
                [obj setValue:@"" forKey:pKey];
            }else if([[properties objectForKey:pKey] isEqualToString:@"NSNumber"]){
                [obj setValue:[NSNumber numberWithInt:0] forKey:pKey];
            }
        }
