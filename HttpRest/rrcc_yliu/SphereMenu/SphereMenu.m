...
static const CGFloat kSphereLength = 100;//80
...
@property (nonatomic, assign) CGPoint itemsCenter;
...
- (instancetype)initWithFrame:(CGRect)frame startImage:(UIImage *)startImage submenuImages:(NSArray *)images
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.itemsCenter = frame.origin;
        self.center = self.itemsCenter;
        [self SetBorderWithcornerRadius:30.f BorderWith:0.f AndBorderColor:nil];// add
        self.userInteractionEnabled = YES;
        self.tapOnStart = [[UITapGestureRecognizer alloc] initWithTarget:self
                                                                  action:@selector(startTapped:)];
        [self addGestureRecognizer:self.tapOnStart];
    }
    return self;
}
...
- (void)commonSetup
{
    ...
    for (int i = 0; i < self.count; i++) {
        ...
        [item SetBorderWithcornerRadius:30.f BorderWith:0.f AndBorderColor:nil];// add
        item.userInteractionEnabled = YES;
        [self.superview addSubview:item];
        
        CGPoint position = [self centerForSphereAtIndex:i];
        item.center = self.itemsCenter;
    ...
    for (int i = 0; i < self.count; i++) {
        UISnapBehavior *snap = [[UISnapBehavior alloc] initWithItem:self.items[i] snapToPoint:self.itemsCenter];
...
//    CGPoint startPoint = self.itemsCenter;
...
- (CGPoint)centerForSphereAtIndex:(int)index // motify
{
    CGFloat firstAngle = M_PI/2 + index * kAngleOffset;
    CGPoint startPoint = self.itemsCenter;
    CGFloat x = startPoint.x + cos(firstAngle) * kSphereLength;
    CGFloat y = startPoint.y - sin(firstAngle) * kSphereLength;
    CGPoint position = CGPointMake(x, y);
...
- (void)startTapped:(UITapGestureRecognizer *)gesture// 废弃
{
...
{
    UISnapBehavior *snap = [[UISnapBehavior alloc] initWithItem:self.items[index] snapToPoint:self.itemsCenter];
    snap.damping = kSphereDamping;
