...
-(BOOL) initWithAppId:(NSString *)app_id
    ...
    mchid           = [NSString stringWithString:mch_id];

    _orderName      = [NSString stringWithString:orderName];
    _orderPrice     = [NSString stringWithString:orderPrice];
...
- ( NSMutableDictionary *)sendPay {
    ...
    [packageParams setObject: _orderNO          forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: [BZ_Tools getIPAddress:YES]
                      forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: _orderPrice       forKey:@"total_fee"];       //订单金额，单位为分
